...
Writing a Gremlin <<dsl,Domain Specific Language>> (DSL) in Go simply requires direct extension of several classes:

* `GraphTraversal` - which exposes the various steps used in traversal writing
* `T__` - which spawns anonymous traversals from steps
* `GraphTraversalSource` - which spawns `GraphTraversal` instances
...
[source,go]
----
social = traversal(SocialTraversalSource).withRemote(DriverRemoteConnection('ws://localhost:8182/gremlin'))
social.persons('marko').knows('josh')
social.persons('marko').youngestFriendsAge()
social.persons().filter(__.createdAtLeast(2)).count()
----
